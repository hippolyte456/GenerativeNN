...
class CasualDilatedConv1D(nn.Module): 
    def __init__(self, in_channels, out_channels, kernel_size, dilation, padding=1):
        super().__init__()
        self.conv1D = nn.Conv1d(in_channels, out_channels, kernel_size, dilation=dilation, padding=padding, bias=False)  #padding = "same"...
        self.ignoreOutIndex = (kernel_size - 1) * dilation

    def forward(self, x):
        return self.conv1D(x)[..., : -1 ]
        #return self.conv1D(x)[..., : -self.ignoreOutIndex ] #why not the same code than at the end of the tutorial ?


"""
"..." Ellipsis is used to indicate that the dimensions before the last one are the same as the input. The last dimension is the one that is being sliced.
 So, the output of the convolution is the same as the input, except that the last dimension is smaller by the amount of the kernel size.
# https://stackoverflow.com/questions/50999977/what-does-the-ellipsis-stand-for-in-pytorch
ça fonctionne toujours si on l'enleve ? non...
"""
